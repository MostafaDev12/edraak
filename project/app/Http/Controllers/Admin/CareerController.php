...
class CareerController extends Controller
{
    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            })  ->addColumn('checkbox', function(Career $data) {
...
<a href="javascript:;" data-href="' . route('admin-prod-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> Delete</a></div></div>';
    ...
    {
        $pro = Career::where('status','=',1)->orderBy('id','desc')->get();

       $cats = Category::where('type','career')->get();

        return view('admin.career.create.physical',compact('cats','pro'));
    }   

    //*** GET Request
    public function status($id1,$id2)
    {
        $data = Career::findOrFail($id1);
        $data->status = $id2;
        $data->update();
    }
    ...
    public function store(Request $request)
    {
       ...
       $messages =[
          ...
          'name.required'                       => trans('career name Required'),
          'name_ar.required'                    => trans('career ArabicName Required'),
    ...
    public function edit($id)
    {
      ...
      $cats = Category::where('type','career')->get();
        $data = Career::findOrFail($id);
            ...
            return view('admin.career.edit.physical',compact('cats','data'));
    } 
...
// Career DELETE ENDS
    ...
    public function ckeckall(Request $request) {
             ...
             if (!empty($request->input('selected_products'))) {
                    ...
                    $products = Career::whereIn('id', $selected_products)
                                        ->update(['status' => 0]);
                                        
              $msg = 'Career Deactivate Successfully.';
    ...
    public function ckeckactivate(Request $request) {
             ...
             $msg = 'Career activate Successfully.';
           return redirect()->back()->with('success',$msg);
         }else{
             return redirect()->back(); 
         }
    ...
    public function ckeckcatalog(Request $request) {
             ...
             $msg = 'Career add to Catalog Successfully.';
           return redirect()->back()->with('success',$msg);
         }else{
             return redirect()->back(); 
         }
