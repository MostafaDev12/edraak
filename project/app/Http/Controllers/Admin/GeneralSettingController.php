    ...
    public function generalupdate(Request $request)
    {
        ...
        $data = Generalsetting::findOrFail(1);
         if ($file = $request->file('wallet_photo'))
            ...
            if(!empty($request->career_emails)){
                if(in_array(null, $request->career_emails) )
            {
                $input['career_emails'] = null;
            
            }
            else
            {
                $input['career_emails'] = implode(',', str_replace(',',' ',$request->career_emails));
            ...
            if(!empty($request->support_emails)){
                if(in_array(null, $request->support_emails) )
            {
                $input['support_emails'] = null;
            
            }
            else
            {
                $input['support_emails'] = implode(',', str_replace(',',' ',$request->support_emails));
            
            }
            }
            if(!empty($request->product_emails)){
                if(in_array(null, $request->product_emails) )
            {
                $input['product_emails'] = null;
            
            }
            else
            {
                $input['product_emails'] = implode(',', str_replace(',',' ',$request->product_emails));
