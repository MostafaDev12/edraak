    ...
    $("#registerform").on('submit', function (e) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data == 1) {
            window.location = mainurl2 + '/user/dashboard';
          } else {
      ...
      $('.'+$selector).hide();
        $.get( $(this).data('href') , function( data ) {
            if(data == 0) {
                ...
                $(".cart-count").html(data);
               $('.cart-table').html('<h3>'+langg.lang8+'</h3>');
                $('#cart-items').html('<div class="dropdownmenu-wrapper"><h3>'+langg.lang8+'</h3></div>');
...
// Adding Muliple Quantity Starts
    ...
    $(document).on('click', '.product-size .siz-lists .boxs', function () {
        $('.qttotal').html('1');
        var parent = $(this).parent();
         ...
         var sign_ar = $('#curr_sign_ar').val();
         if(pos == '0')
         {
         $('#sizeprice').html(sign+total);
         $('#sizeprice_ar').html(sign_ar+total);
         }
         else {
         $('#sizeprice').html(total+sign);
         $('#sizeprice_ar').html(total+sign_ar);
         }
...
$(document).on('change','.product-attr2',function(){

         var total = 0;
         total = getAmount()+getSizePrice2();
         total = total.toFixed(2);
         var pos = $('#curr_pos').val();
         var sign = $('#curr_sign').val();
           var sign_ar = $('#curr_sign_ar').val();
...
function getSizePrice2()
{
  ...
  var total = 0;
  if($('.product-color .color-list2 li').length > 0)
  {
    total = parseFloat($('.product-color .color-list2 li.active').find('.color_price').val());
  }
...
function getAmount()
{
  ...
  return total;
    ...
    $(document).on('click', '.product-color .color-list .boxz', function () {
    ...
    // Product Details Product Size Active Js Code
    $(document).on('click', '.product-size .siz-list2 .boxz', function () {
        $('.qttotal').html('1');
        var parent = $(this).parent();
         size_qty = $(this).find('.size_qty').val();
         size_price = $(this).find('.size_price').val();
  ...
  // Product Details Product Color Active Js Code
    $(document).on('click', '.product-color .color-list-box .boxs', function () {
...
prices = $(".product-attr:checked").map(function() {
 ...
 $(document).on("click", "#maddcrt2" , function(){
     var qty = $('.qttotal').html();
